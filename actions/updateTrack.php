...
if(isset($_POST['submit'])){
    ...
    if (empty($_FILES["image"]["name"])) {
        ...
        if ($image_upload) {
            $addProject = update_track_project_controller($id, $pname, $description, $target_file, $investor, $type, $company, $location, $contact);
            if ($addProject) {
                echo "<script type='text/javascript'> alert('Successfully added Project');
                document.location.href='../admin/posts/project.php';
                </script>";
            } else {
...
if(isset($_GET['del_id'])){
    $id = $_GET['del_id'];
    $result = delete_one_track($id);

    if($result){
        echo "<script type='text/javascript'> alert('Successfully deleted Project');
                window.history.back();
                </script>";
    }else{
